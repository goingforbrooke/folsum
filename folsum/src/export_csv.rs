...
pub fn export_csv(export_file: &Arc<Mutex<Option<PathBuf>>>, extension_counts: &Arc<Mutex<HashMap<String, u32>>>) -> Result<(), &'static str> {
    ...
    thread::spawn(move || {
        ...
        let locked_extension_counts: MutexGuard<'_, HashMap<String, u32>> = extension_counts_copy.lock().unwrap();
        ...
        let export_filename = locked_export_file.as_ref().expect("No path for export file was specified"); 
